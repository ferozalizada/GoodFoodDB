...
-- k
SELECT RAT.NAME,RAT.JOIN_DATE,RAT.REPUTATION,AVG(RAT.FOOD,RAT.MOOD) AS AVG_FOOD_MOOD, RES.NAME, RATING.DATE
FROM RATER AS RAT NATURAL INNER JOIN RATING NATURAL INNER JOIN RESTAURANT AS RES
GROUP BY RAT.NAME,AVG_FOOD_MOOD;

-- l
SELECT RATER.NAME,RAT.REPUTATION,RES.NAME,RAT.DATE
FROM RATING AS RAT NATURAL INNER JOIN RESTAURANT AS RES NATURAL INNER JOIN RATER
WHERE (RAT.USERID IN (SELECT USERID FROM RATING GROUP BY USERID HAVING MAX(FOOD))
    OR RAT.USERID IN (SELECT USERID FROM RATING GROUP BY USERID HAVING MAX(MOOD)));

-- m
SELECT RATER.NAME,RATING.REPUTATION,RATING.COMMENTS,MENUITEM.PRICE,MENUITEM.NAME, COUNT(RATER.USERID,MENUITEM.RESTAURANTID) AS TOTAL_RESTORANT_REVIEWS
FROM RATER NATURAL INNER JOIN RATING NATURAL INNER JOIN RATINGITEM NATURAL INNER JOIN MENUITEM
GROUP BY RATER.USERID,MENUITEM.RESTAURANTID
ORDER BY TOTAL_RESTORANT_REVIEWS;
